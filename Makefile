...
test: manifests generate fmt vet envtest ## Run tests.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" go test $$(go list ./... | grep -v -E '/e2e|/cmd|/test') -v -ginkgo.v -coverprofile cover.out
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name kubettlreaper-builder
	$(CONTAINER_TOOL) buildx use kubettlreaper-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm kubettlreaper-builder
	rm Dockerfile.cross
...
helm: manifests kustomize helmify
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default | $(HELMIFY) -cert-manager-as-subchart charts/kube-ttl-reaper
##################################
